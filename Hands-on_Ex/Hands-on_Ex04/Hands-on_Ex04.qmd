...
```{r}
wm_q <- poly2nb(hunan_join, queen = TRUE)
summary(wm_q)
```
...
```{r}
lag.list <- list(hunan$NAME_3, lag.listw(rswm_q, hunan_join$GDPPC))
...
```{r}
lag_sum <- list(hunan_join$NAME_3, lag.listw(b_weights2, hunan$GDPPC))
