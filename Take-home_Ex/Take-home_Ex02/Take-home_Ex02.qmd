...
We will first use Moran’s I to check whether similar values cluster geographically
...
```{r}
# Apply poly2nb() on simplified single polygons
...
```{r}
# Perform Moran's I test for revenue_all
...
The LISA results show local clusters of provinces with high or low tourism revenue and detect any outliers where a province is significantly different from its neighbors.

```{r}
# Perform Getis-Ord Gi* statistic for revenue_all
hotspot_analysis <- localG(filtered_data_simplified$revenue_all, lw)

# Add hotspot results to the spatial data
