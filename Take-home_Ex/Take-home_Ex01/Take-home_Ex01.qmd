...
```{r, cache = TRUE}
# Assign accident severity and set NA for undefined cases
car_acc_time <- car_acc_time %>%
  mutate(accident_severity = case_when(
    number_of_fatalities >= 1 | number_of_injuries >= 10 ~ "Heavy", 
    number_of_fatalities == 0 & number_of_injuries > 0 ~ "Middle",
    number_of_fatalities == 0 & number_of_injuries == 0 & number_of_vehicles_involved > 0 ~ "Low",  
    TRUE ~ NA_character_  # Set NA for undefined cases
  ))

# Filter out rows where accident_severity is NA
car_acc_time_filtered <- car_acc_time %>%
  filter(!is.na(accident_severity))

# Check the result
print(car_acc_time_filtered %>% select(number_of_fatalities, number_of_injuries, number_of_vehicles_involved, accident_severity))
```

```{r, cache = TRUE}
# Filter for heavy accidents and remove rows with accident_severity not classified
heavy_accidents <- car_acc_time %>%
  filter(accident_severity == "Heavy")  # Keep only rows with 'Heavy'

# Filter for middle accidents and remove rows with accident_severity not classified
middle_accidents <- car_acc_time %>%
  filter(accident_severity == "Middle")  # Keep only rows with 'Middle'

# Filter for low accidents and remove rows with accident_severity not classified
low_accidents <- car_acc_time %>%
  filter(accident_severity == "Low")  # Keep only rows with 'Low'

filtered_accidents <- car_acc_time %>%
  filter(accident_severity %in% c("Heavy", "Middle", "Low"))  # Keep only valid categories
```
