...
Now we’re loading both the event (acc), polygon (dm2) and road network (Road). Ensure that each layer is in the same CRS.
...
```{r, cache = TRUE}
acc_ppp <- as.ppp(acc_with_roads)
```

```{r, eval: false}
# Convert the filtered road network (lixels) geometry to psp and linnet objects
# roads_psp <- as.psp(st_geometry(filtered_lixels), window = as.owin(filtered_lixels))
# Road_bmr_linnet <- as.linnet(roads_psp)
```

```{r eval: fales}
# kfun_bmr_accidents <- kfunctions(network = Road_bmr,   # The road network (linnet)
#                                  events = acc_ppp,      # The accident points (ppp)
#                                  start = 0,                  
#                                  end = 5000,                 
#                                  step = 100,                 
#                                  width = 100,                
