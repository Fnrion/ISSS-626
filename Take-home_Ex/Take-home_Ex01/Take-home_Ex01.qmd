...
```{r, cache = TRUE}
# Assign accident severity without setting NA (only keep defined cases)
car_acc_time_filtered <- car_acc_time %>%
  ...
  filter(!is.na(accident_severity))  # Automatically filters rows where accident_severity is not defined
...
car_acc_time_filtered <- car_acc_time %>%
  filter(!is.na(hour), !is.na(accident_severity))
...
car_acc_time <- car_acc_time %>%
  mutate(time_of_day = case_when(
    hour >= 6 & hour < 18 ~ "Morning",  
    TRUE ~ "Evening"                     
  ))

car_acc_time_filtered <- car_acc_time %>%
  filter(!is.na(hour), !is.na(time_of_day), !is.na(accident_severity))
